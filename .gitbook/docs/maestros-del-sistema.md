...
| - | ------- | --------- | ------------------- | -------------------------- |
| 1 | **Equipos** | Registro central de todos los activos fríos del Mandante (marca, modelo, N° de serie, estado). Se alimenta por **carga masiva** Excel/API; no existe alta individual de equipos. | ✅ Pantalla funcional (`maestro-equipos.html`) — debe conservar solo acción de carga masiva, sin botón "Nuevo equipo" | `equipo` (§2.4) |
| 2 | **Gestores** | Alta y administración de gestores autorizados (RUT, nombre, múltiples direcciones/sucursales, estado, tipo de integración ERP). | ✅ Pantalla funcional (`gestores.html`) — formulario debe soportar múltiples direcciones/sucursales | `gestor` / `gestor_direccion` (§2.3) |
...
| 4 | **Ubicaciones / Bodegas** | Lugares físicos donde puede estar un equipo: bodegas del Mandante, del Gestor, puntos de retiro. Incluye **RUT obligatorio del dueño** de la instalación. | ✅ Pantalla funcional (`ubicaciones-bodegas.html`) — debe agregar RUT obligatorio en tabla/modal | `ubicacion_bodega` (§2.8b) |
...
| 7 | **Plantillas de Inspección** | Listas de verificación configurables para la inspección visual en la recepción de equipos por el Gestor, con ítems de checkbox, texto o fotografía obligatoria. | ✅ Pantalla funcional (`plantillas-inspeccion.html`) — pendiente validar interacción en recepción | `plantilla_inspeccion` / `plantilla_inspeccion_item` (pendiente de modelar) |
...
Los submenús **Grupo de Máquinas**, **Familia de Máquinas**, **Marcas** y **Modelos** quedan eliminados de la navegación del Mandante. Sus datos siguen existiendo como atributos de clasificación importados en la carga masiva de equipos y usados por filtros, reportes y matching venta↔máquina, pero no se administran manualmente como maestros en esta fase.
...
| # | Maestro                   | Propósito                                                                                                                                                                                | Estado en prototipo                                                                                                                                         | Entidad en `data_model.md` |
| - | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| 1 | **Clientes**              | Puntos de venta (clientes finales) del Gestor, sincronizados automáticamente desde su ERP vía API (RN-11/RN-12/RN-19). Es una vista de solo lectura: no permite crear ni editar clientes. | ✅ Pantalla funcional (`clientes.html`) — debe bloquear alta/edición manual | `cliente_final` (§2.3) |
| 2 | **Vendedores**            | Vendedores del Gestor, sincronizados automáticamente desde su ERP vía API. Es una vista de solo lectura: no permite crear/editar vendedores ni asignar clientes manualmente.             | ✅ Pantalla funcional (`vendedores.html`) — debe bloquear CRUD y asignación manual de clientes | `vendedor` (§2.3) |
| 3 | **Ubicaciones / Bodegas** | Bodegas y puntos de retiro propios del Gestor donde recibe, almacena o retira equipos. Incluye RUT obligatorio del dueño de la instalación.                                               | ✅ Pantalla funcional (`distribuidor/ubicaciones-bodegas.html`) — debe agregar RUT obligatorio | `ubicacion_bodega` (§2.8b) |
...
> **Nota (10/07/2026):** **Clientes** y **Vendedores** permanecen visibles en el panel del Gestor, pero cambian a consulta de solo lectura por acuerdo de reunión. La información pertenece al ERP y se sincroniza automáticamente; no se administra manualmente en esta plataforma.
...
| **Gestores** | Asignación de Equipos · Maestro de Equipos · Autorización de Movimientos · Informes · Guías de Despacho · APIs de integración | Identifica a quien recibe y administra los equipos. Incluye tipo de integración ERP y múltiples direcciones/sucursales. Solo gestores activos pueden recibir asignaciones. |
...
| **Plantillas de Inspección** | Recepción de Equipos | Estandariza qué se revisa al aceptar/rechazar; puede exigir checkbox, texto o fotografía según ítem. Pendiente validar UX de aprobación masiva sin observaciones. |
