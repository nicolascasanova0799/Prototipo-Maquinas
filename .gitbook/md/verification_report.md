...
- **Distribuidor**: 1/8 pantallas existen (solo dashboard). 7 pantallas críticas faltantes.
...
| S-03 | Maestro de Equipos | `Prototipo Maquinas/mandante/maestro-equipos.html` | ✅ | Buscador, filtros (Estado, Distribuidor), botones carga manual y "Nuevo equipo" (modal alta individual), tabla con paginación, 7 estados badge, modal detalle equipo, botón editar por fila con modal editable. |
| S-04 | Gestores / Distribuidores | `Prototipo Maquinas/mandante/gestores.html` | ⚠️ | Tabla con RUT/dirección/estado/N° equipos. "Tipo ERP" no es columna propia; aparece como subtítulo bajo el nombre. Modal CRUD presente. |
| S-05 | Asignación de Equipos | `Prototipo Maquinas/mandante/asignacion-equipos.html` | ✅ | Selector distribuidor, tabla equipos con checkbox, panel resumen sticky, modal Guía Despacho con PDF adjunto (RN-16). Soporta modos new/edit/view. |
| S-06 | Autorización de Movimientos | `Prototipo Maquinas/mandante/autorizacion-movimientos.html` | ✅ | Filtro estado, tabla con Equipo/Distribuidor/Tipo/Motivo/Fecha/Estado/Acciones, modal detalle con timeline y evidencia. |
...
| S-08 | Informes | `Prototipo Maquinas/mandante/informes.html` | ✅ | 4 tabs: Geolocalización (mapa con pines), Piramidal Ventas (chart), Rendimiento (chart + tabla), Servicio Técnico (tabla con SLA). Filtros tipo máquina/distribuidor/fechas. Nota RN-14 visible. |
| S-09 | Consulta de Inventario | `Prototipo Maquinas/mandante/consulta-inventario.html` | ✅ | Filtros distribuidor/estado/fechas, tabla con vendedores/comuna, expandable de discrepancias con diff positivo/negativo/cero. |
| S-10 | Dashboard Distribuidor | `Prototipo Maquinas/distribuidor/dashboard.html` | ❌ | Ver nota abajo |
...
- **Selección manual de rol**: ✅ (corregido 06/07/2026) `DESIGN.md` § Screen Login define expresamente el bloque `role-toggle` con las opciones Mandante/Distribuidor, igual que el prototipo. El hallazgo original atribuía a `DESIGN.md` una regla de reconocimiento automático de rol que no está presente en el documento vigente.

### Detalle S-10 (Dashboard Distribuidor)
...
|---|---|---|---|---|
| AUTH-1 | Login diferenciado Mandante/Distribuidor | Must | ✅ | (corregido 06/07/2026) El login usa toggle manual de rol, tal como especifica `DESIGN.md` § Screen Login (`role-toggle`) |
...
| MAN-4 | Maestro de Gestores/Distribuidores | Must | ✅ | `mandante/gestores.html` con tabla, modal CRUD |
...
| MAN-15 | Maestro de Ubicaciones / Bodegas | Should | ✅ | `mandante/ubicaciones-bodegas.html` con tabla, modal CRUD |
...
| ID | Funcionalidad | Prioridad | Estado | Observación |
|---|---|---|---|---|
| MAN-5 | Asignación de equipos a Distribuidor | Must | ✅ | `mandante/asignacion-equipos.html` + `asignaciones.html` (vista lista) |
| MAN-5b | Guía de Despacho (Mandante → Distribuidor) | Must | ✅ | Modal con carga PDF obligatoria, N° documento opcional |
...
### 2.5 Panel Distribuidor — Maestros

| ID | Funcionalidad | Prioridad | Estado | Observación |
|---|---|---|---|---|
| DIS-13 | Maestro de Clientes | Must | 🔲 | No existe `distribuidor/clientes.html` |
| DIS-14 | Maestro de Ubicaciones / Bodegas | Should | 🔲 | No existe `distribuidor/ubicaciones-bodegas.html` |

### 2.6 Panel Distribuidor — Recepción y asignación
...
### 2.7 Panel Distribuidor — Inventario
...
### 2.8 Panel Distribuidor — Reportes e integración
...
| GEN-2 | Guías de Despacho | Must | ✅ | Modal en asignación Mandante con PDF. No se puede verificar lado Distribuidor (pantalla faltante). |
...
|---|---|---|---|
| RN-1 | Distribuidor no puede dar baja unilateral | 🔲 | No verificable: Solicitud de Movimiento (DIS-4) faltante. En Autorización (MAN-6) se ve que el Mandante aprueba/rechaza, pero no se puede verificar el flujo completo del lado Distribuidor. |
...
| RN-3 | Estado cambia a "Asignado al Distribuidor" al asignar (fusión de los antiguos "En Tránsito"/"En Distribuidor", ver `business_rules_and_open_questions.md` §0) | ✅ | Verificable: `asignacion-equipos.html` muestra alerta "Asignación confirmada. Los equipos han pasado a estado 'Asignado al Distribuidor'" y `recepcion-equipos.html` mantiene el mismo estado tras la confirmación de recepción. *(Actualizado 06/07/2026)* |
...
| RN-10 | No gestión de máquinas propias del Distribuidor | ✅ | Maestro de Equipos muestra equipos del Mandante; no hay gestión de equipos propios en sidebar distribuidor |
...
| 6 | Ubicaciones / Bodegas | Verificar | `mandante/ubicaciones-bodegas.html` | ✅ | Tabla con nombre, dirección, tipo, responsable, estado, modal CRUD |
...
| 2 | Ubicaciones / Bodegas | Verificar | `distribuidor/ubicaciones-bodegas.html` | 🔲 | Archivo no existe. Link en sidebar apunta a `#` |
...
| Asignado al Distribuidor (antes "En Tránsito"/"En Distribuidor") | #E3F2FD fondo / navy texto | `.badge-distribuidor { background: #E3F2FD; color: var(--azul-marino) }` | ✅ |
...
|---|---|---|---|
| Marcela (Mandante) | Genera asignación → recibe notificación de reparo → autoriza/rechaza | ⚠️ | Flujo de asignación ✅ (`asignacion-equipos.html`), autorización ✅ (`autorizacion-movimientos.html`). Notificación de reparo no verificable (Recepción del Distribuidor faltante). Flujo parcial. |
| Rodrigo (Distribuidor) | Recibe lote → inspecciona → acepta/rechaza → asigna a cliente → solicita movimiento | ⚠️ | **Parcialmente verificable**: Recepción ✅ (`recepcion-equipos.html`), asignación a cliente ✅ (`asignacion-clientes.html` + `asignaciones-realizadas.html`). Solicitud de movimiento (DIS-4) faltante. |
...
| `distribuidor/guias-despacho.html` (link en sidebar) | ❌ No documentado | — | Link presente en sidebar del distribuidor pero el archivo no existe. No está en el checklist §5.2. **Pantalla no documentada y no implementada.** |

---

## Hallazgos Críticos

### 🔴 CRÍTICO 1: Panel Distribuidor incompleto (6 pantallas faltantes)

**Impacto**: 6 de 8 pantallas del Distribuidor no existen. Esto representa el **75% del panel Distribuidor** sin implementar.
...
### 🔴 CRÍTICO 2: Maestros del Distribuidor faltantes
...
`DESIGN.md` § Screen Login ya especifica el `role-toggle` manual (Mandante/Distribuidor), igual que el prototipo. No existe contradicción: el hallazgo original citaba una regla que no está en la versión vigente del documento.
...
### ✅ RESUELTO 06/07/2026 — KPIs del Dashboard Distribuidor
...
### 🟡 IMPORTANTE 4: Navegación del sidebar del Distribuidor con diferencias de estructura

- Distribuidor: "Recepción" en "Principal" (esperado "Operación"), "Reportes" en "Análisis" (esperado "Operación")
- Distribuidor: "Asignación a Clientes" en "Principal" — ✅ (corregido 06/07/2026) ahora alineada con `stitch_prompts.md` §3.3a/3.3b y `system_diagrams.md`
- Distribuidor: "Sincronización de Clientes" y "Notificación de Ventas" **ausentes del sidebar**
- Distribuidor: "Guías de Despacho" **presente pero no documentada**
...
1. **Implementar las 6 pantallas faltantes del panel Distribuidor**. Estas son críticas para validar los flujos de negocio principales (solicitudes, inventario, sincronización, notificación de ventas). Sin estas pantallas, el 50% del sistema no es navegable.

2. **Implementar `distribuidor/clientes.html`** (Maestro de Clientes, prioridad Must). Es el único maestro Must del distribuidor y no existe.
...
6. ~~**Sincronizar KPIs del Dashboard Distribuidor**~~ — **Resuelto 06/07/2026**: `stitch_prompts.md` §3.1 ya coincide con el prototipo.

7. **Añadir "Sincronización de Clientes" y "Notificación de Ventas" al sidebar del Distribuidor**. Ambas son funcionalidades Must y no aparecen en la navegación.

### Prioridad Media

8. **Decidir sobre "Guías de Despacho" en sidebar Distribuidor**: Si es una pantalla válida, documentarla y crear el archivo. Si no, remover el link del sidebar.
...
10. **Considerar documentar el gráfico doughnut** del Dashboard Distribuidor en `stitch_prompts.md` o `DESIGN.md`, ya que está implementado pero no especificado.

11. ~~**Actualizar `DESIGN.md` Screen Login**~~ — **Resuelto 06/07/2026**: ya usa RUT, coincidiendo con el prototipo.

### Prioridad Baja

12. **Estandarizar la estructura de secciones del sidebar del Distribuidor**: Decidir si "Recepción" y "Asignación a Clientes" van en "Principal" o "Operación", y si "Reportes" va en "Análisis" o "Operación" — para el Mandante ya está resuelto (ver §5.1). Actualizar el checklist §5 en `skills/prototype-verification/assets/` en consecuencia.
